...
export default function AppSidebar() {
  ...
  const navCls = (isActive) => `group flex items-center gap-2.5 px-3 py-1.5 rounded-md text-sm transition-colors ${isActive ? 'bg-accent text-accent-foreground' : 'text-muted-foreground hover:bg-accent'}`;

  return (
    <div className="w-60 flex-shrink-0 flex flex-col border-r bg-card border-border z-10" data-testid="app-sidebar">
      ...
      <div className="flex-1 overflow-y-auto px-2 py-3 space-y-5">
        <div>
          <div className="px-2 mb-1 text-[10px] uppercase tracking-widest text-muted-foreground font-semibold">Workspace</div>
        ...
        <div>
          <div className="px-2 mb-1 flex items-center justify-between">
            <span className="text-[10px] uppercase tracking-widest text-muted-foreground font-semibold">Projects</span>
            <Dialog open={showCreate} onOpenChange={setShowCreate}>
              <DialogTrigger asChild>
                <button data-testid="create-project-btn" className="h-5 w-5 rounded flex items-center justify-center text-muted-foreground hover:t-text transition-colors">
                  <Plus size={12} />
                </button>
              </DialogTrigger>
              <DialogContent className="sm:max-w-sm bg-popover border">
                ...
                <div className="space-y-3 mt-2">
                  <input data-testid="project-name-input" value={newName} onChange={e => { setNewName(e.target.value); if (!newKey) setNewKey(e.target.value.slice(0,3).toUpperCase()); }}
                    placeholder="Project name" className="flex h-9 w-full rounded-md border-input bg-transparent text-foreground placeholder:text-muted-foreground px-3 text-sm focus:outline-none focus:ring-1 focus:ring-indigo-400" />
                  <input data-testid="project-key-input" value={newKey} onChange={e => setNewKey(e.target.value.toUpperCase())}
                    placeholder="Key (e.g. FE)" className="flex h-9 w-full rounded-md border-input bg-transparent text-foreground placeholder:text-muted-foreground px-3 text-sm focus:outline-none focus:ring-1 focus:ring-indigo-400" />
                  ...
                  <button data-testid="submit-project-btn" onClick={handleCreate} className="h-9 w-full rounded-md bg-primary text-primary-foreground text-sm font-medium transition-colors">Create Project</button>
                </div>
              </DialogContent>
            </Dialog>
          </div>
          <nav className="space-y-0.5">
              ...
              <div key={p.id}>
                ...
                {projectId === p.id && (
                  <div className="ml-8 mt-0.5 space-y-0.5">
                    <NavLink to={`/project/${p.id}/board`} end className={({ isActive }) => `block text-xs py-1 px-2 rounded transition-colors ${isActive ? 't-text text-indigo-400' : 'text-muted-foreground hover:text-muted-foreground'}`}>
                      <FolderKanban size={12} className="inline mr-1.5" />Sprint Board
                    </NavLink>
                    <NavLink to={`/project/${p.id}/bugs`} className={({ isActive }) => `block text-xs py-1 px-2 rounded transition-colors ${isActive ? 't-text text-indigo-400' : 'text-muted-foreground hover:text-muted-foreground'}`}>
            ...
            {projects.length === 0 && <p className="text-xs text-muted-foreground px-3 py-2">No projects yet</p>}
          </nav>
        </div>
      </div>
      ...
      <div className="px-3 py-3 border-t border-border flex items-center justify-between">
        ...
        <button onClick={() => { logout(); }} data-testid="logout-btn" className="h-7 w-7 rounded flex items-center justify-center text-muted-foreground hover:t-text transition-colors">
          <LogOut size={14} />
        </button>
      </div>
    </div>
