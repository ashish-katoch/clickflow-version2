...
export default function DetailPanel({ data, onClose }) {
  ...
  return (
    <div className="absolute inset-y-0 right-0 w-[480px] theme-bg border-l theme-border shadow-2xl z-50 slide-in-right flex flex-col" data-testid="detail-panel">
      {/* Header */}
      <div className="h-12 flex items-center justify-between px-4 border-b theme-border flex-shrink-0">
        <div className="flex items-center gap-2">
          <span className="text-[10px] theme-text-muted font-mono">{entity.key}</span>
          ...
          {statusInfo && <span className="flex items-center gap-1 text-[10px] theme-text-secondary"><span className={`w-1.5 h-1.5 rounded-full ${statusInfo.dot}`} />{statusInfo.label}</span>}
        </div>
        <button onClick={onClose} data-testid="close-detail" className="h-7 w-7 rounded flex items-center justify-center theme-text-muted hover:theme-text hover:bg-zinc-800/50 transition-colors">
          <X size={16} />
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-5">
        ...
        <input value={entity.title || ''} onChange={e => setEntity({ ...entity, title: e.target.value })}
          ...
          className="w-full bg-transparent text-lg font-semibold theme-text outline-none border-none" style={{ fontFamily: 'Manrope' }}
          data-testid="detail-title" />

        {/* Properties grid */}
        <div className="space-y-3">
          <div className="grid grid-cols-[100px_1fr] items-center gap-2">
            <label className="text-[11px] theme-text-muted font-medium">Status</label>
            <Select value={entity.status} onValueChange={v => { setEntity({ ...entity, status: v }); handleUpdate('status', v); }}>
              <SelectTrigger className="h-8 theme-border bg-transparent theme-text text-sm" data-testid="detail-status"><SelectValue /></SelectTrigger>
          ...
          <div className="grid grid-cols-[100px_1fr] items-center gap-2">
            <label className="text-[11px] theme-text-muted font-medium">Priority</label>
            <Select value={entity.priority} onValueChange={v => { setEntity({ ...entity, priority: v }); handleUpdate('priority', v); }}>
              <SelectTrigger className="h-8 theme-border bg-transparent theme-text text-sm" data-testid="detail-priority"><SelectValue /></SelectTrigger>
              <SelectContent className="theme-dropdown border">
          ...
          <div className="grid grid-cols-[100px_1fr] items-center gap-2">
            <label className="text-[11px] theme-text-muted font-medium">Assignee</label>
            <Select value={entity.assignee_id || '__none__'} onValueChange={v => { const val = v === '__none__' ? null : v; setEntity({ ...entity, assignee_id: val }); handleUpdate('assignee_id', val); }}>
              <SelectTrigger className="h-8 theme-border bg-transparent theme-text text-sm" data-testid="detail-assignee"><SelectValue placeholder="Unassigned" /></SelectTrigger>
              <SelectContent className="theme-dropdown border">
          ...
          {isTask && (
            <div className="grid grid-cols-[100px_1fr] items-center gap-2">
              <label className="text-[11px] theme-text-muted font-medium">Due Date</label>
              <input type="date" value={entity.due_date || ''} onChange={e => { setEntity({ ...entity, due_date: e.target.value }); handleUpdate('due_date', e.target.value || null); }}
                className="h-8 w-full rounded-md border theme-border bg-transparent px-3 text-sm theme-text focus:outline-none focus:ring-1 focus:ring-zinc-400"
          ...
          {assignee && (
            <div className="grid grid-cols-[100px_1fr] items-center gap-2">
              <label className="text-[11px] theme-text-muted font-medium">Assigned</label>
              <div className="flex items-center gap-2">
        ...
        {/* Description */}
        <div>
          <label className="text-[11px] theme-text-muted font-medium block mb-1.5">Description</label>
          <textarea value={entity.description || ''} onChange={e => setEntity({ ...entity, description: e.target.value })}
            ...
            className="w-full min-h-[80px] rounded-md border theme-border bg-transparent px-3 py-2 text-sm theme-text placeholder:theme-text-muted focus:outline-none focus:ring-1 focus:ring-zinc-400 resize-none"
        ...
        {!isTask && (
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-[11px] theme-text-muted font-medium">Attachments ({(entity.attachments || []).length})</label>
              <div className="flex items-center gap-1">
                <label className="cursor-pointer h-6 px-2 rounded text-[10px] theme-text-secondary hover:theme-text flex items-center gap-1 hover:bg-zinc-800/50 transition-colors" data-testid="upload-file-btn">
                  <input type="file" accept="image/*" onChange={handleFileUpload} className="hidden" />
                  <ImagePlus size={12} /> Upload
                </label>
                <button onClick={() => setShowAttachInput(!showAttachInput)} className="h-6 px-2 rounded text-[10px] theme-text-secondary hover:theme-text hover:bg-zinc-800/50 transition-colors" data-testid="add-url-btn">
                  + URL
                </button>
              </div>
            </div>
            {showAttachInput && (
              <div className="flex gap-2 mb-2">
                <input value={attachUrl} onChange={e => setAttachUrl(e.target.value)} placeholder="Paste image URL..."
                  className="flex-1 h-8 rounded-md border theme-border bg-transparent px-3 text-sm theme-text placeholder:theme-text-muted focus:outline-none focus:ring-1 focus:ring-zinc-400"
                  data-testid="attachment-url-input" />
                <button onClick={() => handleAddAttachment(attachUrl, 'image')} className="h-8 px-3 rounded-md theme-btn-primary text-xs font-medium" data-testid="submit-url-btn">Add</button>
              </div>
            )}
            {(entity.attachments || []).length > 0 && (
              <div className="grid grid-cols-2 gap-2">
                {(entity.attachments || []).map(att => (
                  <div key={att.id} className="relative group rounded-lg overflow-hidden border theme-border" data-testid={`attachment-${att.id}`}>
            ...
            {(entity.attachments || []).length === 0 && !showAttachInput && (
              <p className="text-xs theme-text-muted">No attachments. Upload images or paste URLs.</p>
            )}
          </div>
        )}
        ...
        {/* Activity & Comments */}
        <div>
          <label className="text-[11px] theme-text-muted font-medium block mb-2">Activity & Comments</label>
          <div className="space-y-3 mb-3 max-h-60 overflow-y-auto">
            {comments.map(c => (
              <div key={c.id} className={`flex gap-2.5 ${c.type === 'activity' ? 'opacity-60' : ''}`} data-testid={`comment-${c.id}`}>
                <div className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 ${c.type === 'activity' ? 'theme-chip' : getAvatarColor(c.user_name)}`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] theme-text-muted">{new Date(c.created_at).toLocaleString('en', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</span>
                  </div>
                  <p className={`text-xs mt-0.5 ${c.type === 'activity' ? 'theme-text-muted italic' : 'theme-text-secondary'}`}>{c.content}</p>
                </div>
              </div>
            ))}
            {comments.length === 0 && <p className="text-xs theme-text-muted">No activity yet</p>}
          </div>
          {/* Comment input */}
          <div className="flex gap-2">
            ...
            <button onClick={handleComment} data-testid="submit-comment" className="h-8 w-8 rounded-md flex items-center justify-center theme-text-secondary hover:theme-text hover:bg-zinc-800/50 transition-colors">
              <Send size={14} />
            </button>
          </div>
        </div>
