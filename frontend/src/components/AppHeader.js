...
export default function AppHeader() {
  ...
  return (
    <header className="h-14 flex-shrink-0 bg-background/80 border-border border-b backdrop-blur-md sticky top-0 z-10 flex items-center justify-between px-6" data-testid="app-header">
      {/* Search */}
      <div className="relative flex-1 max-w-lg">
        <div className="flex items-center gap-2">
          ...
          {searchQuery && <button onClick={() => { setSearchQuery(''); setSearchResults(null); }} className="text-muted-foreground hover:t-text"><X size={14} /></button>}
        </div>
        {/* Search Results Dropdown */}
        {searchResults && (
          <div className="absolute top-full left-0 right-0 mt-2 bg-popover border rounded-lg overflow-hidden z-50 max-h-80 overflow-y-auto" data-testid="search-results">
            {searchResults.tasks.length === 0 && searchResults.bugs.length === 0 ? (
              <p className="text-xs text-muted-foreground p-4 text-center">No results for "{searchQuery}"</p>
            ) : (
              <>
                {searchResults.tasks.length > 0 && (
                  <div>
                    <div className="px-3 py-1.5 text-[10px] uppercase tracking-widest text-muted-foreground font-semibold border-b border-border">Tasks</div>
                ...
                {searchResults.bugs.length > 0 && (
                  <div>
                    <div className="px-3 py-1.5 text-[10px] uppercase tracking-widest text-muted-foreground font-semibold border-b border-border">Bugs</div>
                    {searchResults.bugs.map(b => (
                      <button key={b.id} onClick={() => { navigate(`/project/${b.project_id}/bugs`); setSearchQuery(''); setSearchResults(null); }}
                        ...
                        <span className="text-[10px] text-muted-foreground font-mono">{b.key}</span>
      ...
      <div className="flex items-center gap-1.5">
        ...
        <button onClick={toggleTheme} data-testid="theme-toggle-btn"
          className="h-8 w-8 rounded-md flex items-center justify-center text-muted-foreground hover:t-text transition-colors"
        ...
        <Popover open={notifsOpen} onOpenChange={handleNotifsOpen}>
          <PopoverTrigger asChild>
            <button className="h-8 w-8 rounded-md flex items-center justify-center text-muted-foreground hover:t-text transition-colors relative" data-testid="notifications-btn">
              <Bell size={16} />
          ...
          <PopoverContent className="w-80 p-0 bg-popover border" align="end">
            <div className="flex items-center justify-between px-3 py-2 border-b border-border">
              <span className="text-[10px] font-semibold text-muted-foreground uppercase tracking-widest">Notifications</span>
            ...
            <div className="max-h-64 overflow-y-auto">
              {notifs.length === 0 ? <p className="text-xs text-muted-foreground p-4 text-center">No notifications</p> : notifs.map(n => (
                <div key={n.id} className={`px-3 py-2.5 border-b border-border text-xs ${n.read ? 'text-muted-foreground' : 't-text'}`}>
                  <div>{n.message}</div>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent className="w-56 bg-popover border" align="end">
            <div className="px-3 py-2.5">
              <div className="text-sm font-medium t-text">{user?.name || 'User'}</div>
              <div className="text-xs text-muted-foreground mt-0.5">{user?.email}</div>
              <div className="text-[10px] mt-1 px-1.5 py-0.5 rounded inline-block bg-secondary text-secondary-foreground capitalize">{user?.role || 'member'}</div>
            </div>
